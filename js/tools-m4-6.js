...
"layout_prompt": "برومبت تخطيط إضافى بالإنجليزية يصف مواضع العناصر بدقة داخل إطار 16:9 (مثال: subject on right third with face 40% of frame looking left, bold text zone on left third upper area, key object bottom-left, negative space top-center, rule of thirds, safe margins for YouTube duration badge bottom-right) — يُستخدم كمرجع بجانب البرومبت الرئيسى",
...
"tips": ["3 نصائح لتحسين النتيجة فى أداة التوليد"]}` + Ctx.kb(),
    ...
    const html = `
      ...
      ${ltr('<i class="fa-solid fa-table-cells-large"></i> برومبت التخطيط (مواضع العناصر) — استخدمه كمرجع مع البرومبت الرئيسى لأفضل نتيجة', j.layout_prompt)}
...
{
  id: 'trends-radar', module: 'm4', icon: 'fa-satellite-dish',
  name: 'رادار الترندات', en: 'Trends Radar',
  desc: 'بحث حى فى الإنترنت + يوتيوب عن أحدث ترندات النيتش الآن، مع اقتراح أفكار فيديوهات جاهزة لركوب الموجة.',
  apis: ['yt', 'ai'],
  fields: [
    F.niche(),
    F.select('period', 'نطاق الحداثة', ['آخر أسبوع', 'آخر أسبوعين', 'آخر شهر'], 'آخر أسبوع'),
    F.select('depth', 'عمق التحليل', ['سريع', 'متعمق'], 'متعمق')
  ],
  async run(vals, tool, progress) {
    const days = vals.period === 'آخر أسبوع' ? 7 : vals.period === 'آخر أسبوعين' ? 14 : 30;
    const after = new Date(Date.now() - days * 86400000).toISOString();

    // 1) بحث يوتيوب عن أحدث الفيديوهات فى النيتش
    progress('جارِ مسح يوتيوب عن أحدث فيديوهات النيتش...');
    let ytText = '';
    try {
      const res = await YT.search(vals.niche, { maxResults: 25, order: 'viewCount', publishedAfter: after });
      const ids = (res.items || []).map(i => i.id.videoId).filter(Boolean);
      const vids = await YT.videosByIds(ids);
      const sums = vids.map(YT.summarizeVideo).sort((a, z) => z.views - a.views);
      ytText = sums.map(v => `- [${v.published}] "${v.title}" | مشاهدات:${v.views} | تفاعل:${v.likes}👍/${v.comments}💬 | ${v.isShort ? 'Short' : 'طويل'}`).join('\n');
    } catch (e) { ytText = '(تعذر البحث فى يوتيوب: ' + e.message + ')'; }

    // 2) بحث إنترنت حى عبر Google Search Grounding
    progress('جارِ البحث الحى فى الإنترنت عن الترندات...');
    let webText = '';
    try {
      webText = await Gemini.auto(
        `ابحث فى الإنترنت الآن عن أحدث الترندات والمواضيع الساخنة فى مجال "${vals.niche}" خلال ${vals.period}. اذكر: الأحداث والأخبار الجديدة، المواضيع المتصاعدة على السوشيال ميديا، المنتجات/التحديثات الجديدة، وما يتحدث عنه الجمهور حالياً. أعطنى قائمة مركزة بالحقائق مع ذكر حداثة كل معلومة.` + AI.dateNote(),
        { search: true, tool: tool.id, temperature: 0.4 });
    } catch (e) { webText = '(تعذر البحث فى الإنترنت: ' + e.message + ')'; }

    // 3) دمج وتحليل نهائى
    progress('جارِ تحليل الترندات واقتراح الأفكار...');
    const text = await Gemini.generate(
      `أنت رادار ترندات لقناة يوتيوب فى نيتش "${vals.niche}". أمامك مصدران حديثان (${vals.period}):

--- نتائج بحث الإنترنت الحى ---
${webText}

--- أعلى فيديوهات يوتيوب الجديدة فى النيتش (مرتبة بالمشاهدات) ---
${ytText}

المطلوب (${vals.depth}):
1) **خريطة الترندات الآن**: جدول بأهم 5-8 ترندات نشطة | مصدر الترند (إنترنت/يوتيوب/كلاهما) | درجة السخونة 0-100 | المدة المتوقعة لبقاء الترند
2) **تحليل الفجوة**: ما الذى يتصدر ولا يغطيه صناع المحتوى بشكل كافٍ؟ (فرص ذهبية)
3) **10 أفكار فيديوهات جاهزة لركوب الموجة**: لكل فكرة — العنوان المقترح + الزاوية المميزة + نوعه (Short/طويل) + درجة إلحاح النشر + لماذا ستنجح
4) **ترتيب التنفيذ**: أى 3 أفكار أنفذها هذا الأسبوع بالترتيب ولماذا
5) **تحذيرات**: ترندات تبدو مغرية لكنها فخ (متأخرة/مشبعة/خارج النيتش)` + Ctx.kb(),
      { system: SYS_PROMPT, tool: tool.id });
    return { text, html: UI.md(text) };
  }
},
...
{
  id: 'video-info-editor', module: 'm2', icon: 'fa-pen-to-square',
  name: 'محرر معلومات الفيديو', en: 'Video Info Editor',
  desc: 'اعرض معلومات فيديوك، قيّمها بالذكاء الاصطناعى، عدّل العنوان والوصف والثمبنيل، حلل قبل/بعد، ثم طبّق التعديلات فعلياً على يوتيوب.',
  apis: ['yt', 'ai'],
  fields: [F.video()],
  async run(vals, tool, progress) {
    progress('جارِ تحميل بيانات الفيديو...');
    const v = await YT.video(vals.video);
    const sn = v.snippet;
    const stats = v.statistics || {};
    const tags = sn.tags || [];
    const thumbUrl = sn.thumbnails?.medium?.url || sn.thumbnails?.default?.url || '';

    progress('جارِ تقييم المعلومات الحالية بالذكاء الاصطناعي...');
    const j = await Gemini.generateJSON(
      `قيّم معلومات فيديو اليوتيوب التالية وقدّم اقتراحات تحسين. أعد JSON فقط:
الفيديو: "${sn.title}"
تاريخ النشر: ${sn.publishedAt?.slice(0, 10)} | مشاهدات: ${stats.viewCount} | إعجابات: ${stats.likeCount} | تعليقات: ${stats.commentCount}
الوصف الحالى:
${(sn.description || '(فارغ)').slice(0, 1500)}
الوسوم الحالية: ${tags.join(', ') || '(لا توجد)'}

{"title_score": 0-100, "title_notes": "تقييم العنوان الحالى بإيجاز",
"desc_score": 0-100, "desc_notes": "تقييم الوصف",
"tags_score": 0-100, "tags_notes": "تقييم الوسوم",
"suggested_title": "أفضل عنوان مقترح (نفس لغة العنوان الأصلى)",
"alt_titles": ["عنوانان بديلان"],
"suggested_description": "وصف محسّن كامل جاهز للنسخ (نفس لغة الوصف الأصلى، مع الحفاظ على أى روابط موجودة)",
"suggested_tags": ["قائمة وسوم محسّنة كاملة"],
"priority_fix": "أهم تعديل واحد يجب فعله فوراً ولماذا"}` + Ctx.kb(),
      { system: SYS_PROMPT, tool: tool.id, maxTokens: 8192 });
    ...
    const html = `
      <div class="card">
        <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:flex-start">
          ${thumbUrl ? `<img src="${esc(thumbUrl)}" alt="الثمبنيل الحالى" style="width:200px;border-radius:10px;border:1px solid var(--border)">` : ''}
          <div style="flex:1;min-width:220px">
            <h3 style="margin-top:0">${esc(sn.title)}</h3>
            <p class="sub">نُشر: ${esc(sn.publishedAt?.slice(0, 10) || '')} • ${UI.num(stats.viewCount)} مشاهدة • ${UI.num(stats.likeCount)} إعجاب • التصنيف: ${esc(sn.categoryId || '')}</p>
            <div style="display:flex;gap:14px;flex-wrap:wrap;margin-top:8px">
              ${UI.scoreRing(j.title_score ?? 0, 'العنوان', 90)}
              ${UI.scoreRing(j.desc_score ?? 0, 'الوصف', 90)}
              ${UI.scoreRing(j.tags_score ?? 0, 'الوسوم', 90)}
            </div>
          </div>
        </div>
      </div>

      <div class="notice orange" style="margin-top:14px"><i class="fa-solid fa-bolt"></i> <strong>أولوية التعديل:</strong> ${esc(j.priority_fix || '')}</div>

      <div class="grid cols-3" style="margin-top:14px">
        <div class="card"><h3><i class="fa-solid fa-heading"></i> العنوان</h3><p class="sub">${esc(j.title_notes || '')}</p></div>
        <div class="card"><h3><i class="fa-solid fa-align-right"></i> الوصف</h3><p class="sub">${esc(j.desc_notes || '')}</p></div>
        <div class="card"><h3><i class="fa-solid fa-tags"></i> الوسوم</h3><p class="sub">${esc(j.tags_notes || '')}</p></div>
      </div>

      <div class="card" style="margin-top:14px">
        <h3><i class="fa-solid fa-lightbulb"></i> اقتراحات الذكاء الاصطناعي</h3>
        ${UI.copyList([j.suggested_title || '', ...(j.alt_titles || [])], 'عناوين مقترحة')}
        <h3 style="margin-top:12px">وصف مقترح</h3>
        <div class="copy-item"><span style="white-space:pre-wrap">${esc(j.suggested_description || '')}</span>
          <button class="btn btn-secondary btn-sm" data-copy><i class="fa-regular fa-copy"></i></button></div>
        <h3 style="margin-top:12px">وسوم مقترحة</h3>
        ${UI.copyList([(j.suggested_tags || []).join(', ')])}
      </div>

      <div class="card" style="margin-top:14px">
        <h3><i class="fa-solid fa-pen-to-square"></i> التعديل والتطبيق على يوتيوب</h3>
        <p class="sub">عدّل ما تريد ثم حلّل قبل/بعد، وأخيراً طبّق التعديلات على الفيديو الفعلى. يتطلب التطبيق OAuth Client ID فى الإعدادات وتسجيل دخول بحساب صاحب القناة.</p>
        <div class="tool-form" style="margin-top:12px">
          <label class="field" style="grid-column:1/-1"><span>العنوان الجديد</span>
            <input type="text" id="vie-title" value="${esc(sn.title)}" maxlength="100">
            <small class="sub"><span id="vie-title-count">${sn.title.length}</span>/100 حرف</small></label>
          <label class="field" style="grid-column:1/-1"><span>الوصف الجديد</span>
            <textarea id="vie-desc" rows="7">${esc(sn.description || '')}</textarea></label>
          <label class="field" style="grid-column:1/-1"><span>الوسوم الجديدة (مفصولة بفواصل)</span>
            <textarea id="vie-tags" rows="2">${esc(tags.join(', '))}</textarea></label>
          <div class="field" style="grid-column:1/-1"><span>ثمبنيل جديد (اختيارى — حتى 2MB بصيغة JPG/PNG)</span>
            <div class="dropzone" id="vie-thumb"><i class="fa-solid fa-cloud-arrow-up"></i><p>اضغط أو اسحب الثمبنيل الجديد هنا</p></div></div>
        </div>
        <div class="actions-bar">
          <button type="button" class="btn btn-secondary" id="vie-fill"><i class="fa-solid fa-wand-magic-sparkles"></i> املأ باقتراحات الذكاء</button>
          <button type="button" class="btn btn-secondary" id="vie-diff"><i class="fa-solid fa-code-compare"></i> تحليل قبل/بعد</button>
          <button type="button" class="btn btn-primary" id="vie-apply"><i class="fa-brands fa-youtube"></i> تطبيق التعديلات على يوتيوب</button>
        </div>
        <div id="vie-diff-result" style="margin-top:12px"></div>
      </div>`;

    const after = () => {
      const $ = id => document.getElementById(id);
      let newThumb = null;
      UI.bindDropzone($('vie-thumb'), img => {
        if (img.base64.length * 0.75 > 2 * 1024 * 1024) { UI.toast('الثمبنيل يجب أن يكون أقل من 2MB (حد يوتيوب)', 'warning', 4500); return; }
        newThumb = img;
      });
      $('vie-title').addEventListener('input', e => { $('vie-title-count').textContent = e.target.value.length; });

      $('vie-fill').addEventListener('click', () => {
        if (j.suggested_title) $('vie-title').value = j.suggested_title;
        if (j.suggested_description) $('vie-desc').value = j.suggested_description;
        if (j.suggested_tags?.length) $('vie-tags').value = j.suggested_tags.join(', ');
        $('vie-title-count').textContent = $('vie-title').value.length;
        UI.toast('تم ملء الحقول باقتراحات الذكاء ✓', 'success');
      });

      $('vie-diff').addEventListener('click', async () => {
        const el = $('vie-diff-result');
        el.innerHTML = UI.inlineLoader('جارِ تحليل قبل/بعد...');
        try {
          const diff = await Gemini.generate(
            `قارن بين النسخة القديمة والجديدة لمعلومات فيديو يوتيوب وقيّم هل التعديل تحسين فعلاً:

== قبل ==
العنوان: ${sn.title}
الوصف: ${(sn.description || '').slice(0, 1200)}
الوسوم: ${tags.join(', ')}

== بعد ==
العنوان: ${$('vie-title').value}
الوصف: ${$('vie-desc').value.slice(0, 1200)}
الوسوم: ${$('vie-tags').value}

المطلوب:
1) جدول مقارنة: العنصر | قبل (درجة) | بعد (درجة) | التغيير
2) ما الذى تحسّن تحديداً وما الذى ساء أو ضاع (كلمات مفتاحية فُقدت؟ روابط حُذفت؟)
3) توقع التأثير على CTR والبحث
4) حكم نهائى: طبّق التعديل / عدّل أولاً (مع ما يجب تعديله)`, { system: SYS_PROMPT, tool: 'video-info-editor' });
          el.innerHTML = UI.md(diff);
        } catch (e) { el.innerHTML = `<div class="notice red"><i class="fa-solid fa-circle-xmark"></i> ${UI.esc(e.message)}</div>`; }
      });

      $('vie-apply').addEventListener('click', async () => {
        const newTitle = $('vie-title').value.trim();
        if (!newTitle) return UI.toast('العنوان لا يمكن أن يكون فارغاً', 'warning');
        if (!Store.settings().oauthClientId) return UI.toast('أضف OAuth Client ID فى الإعدادات أولاً لتفعيل التعديل', 'warning', 5000);
        if (!confirm('سيتم تعديل الفيديو فعلياً على يوتيوب:\n\n"' + newTitle + '"\n\nهل أنت متأكد؟')) return;
        const btn = $('vie-apply');
        btn.disabled = true;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> جارِ التطبيق...';
        try {
          // مهم: نرسل snippet كاملاً مع categoryId الأصلى وإلا يرفض يوتيوب
          const snippet = {
            title: newTitle,
            description: $('vie-desc').value,
            tags: $('vie-tags').value.split(',').map(t => t.trim()).filter(Boolean).slice(0, 60),
          ...
          await YT.updateVideo(v.id, snippet);
          if (newThumb) {
            UI.toast('تم تحديث المعلومات ✓ جارِ رفع الثمبنيل...', 'info');
            await YT.setThumbnail(v.id, newThumb.base64, newThumb.mime);
          }
          Store.addAudit('محرر معلومات الفيديو', 'تعديل فيديو على يوتيوب: ' + v.id, 'ok');
          UI.toast('تم تطبيق التعديلات على يوتيوب بنجاح ✓✓', 'success', 6000);
        } catch (e) {
          Store.addAudit('محرر معلومات الفيديو', 'فشل تعديل فيديو: ' + v.id, 'فشل');
          UI.toast('فشل التطبيق: ' + e.message, 'error', 8000);
        } finally {
          btn.disabled = false;
          btn.innerHTML = '<i class="fa-brands fa-youtube"></i> تطبيق التعديلات على يوتيوب';
        }
      });
    };

    const text = `تقييم "${sn.title}":\nالعنوان ${j.title_score}/100 — ${j.title_notes}\nالوصف ${j.desc_score}/100 — ${j.desc_notes}\nالوسوم ${j.tags_score}/100 — ${j.tags_notes}\n\nأولوية: ${j.priority_fix}\n\nعنوان مقترح: ${j.suggested_title}\n\nوصف مقترح:\n${j.suggested_description}\n\nوسوم: ${(j.suggested_tags || []).join(', ')}`;
